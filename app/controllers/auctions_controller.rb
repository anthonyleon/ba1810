  ...
  private
    ...
    def notify(message)
      ...
      parts.each do |part|
        ...
        Notification.create(company_id: part.company.id, auction_id: @auction.id, message: message) unless !@condition.include?(part.condition) || part.company != current_user
      end
    end
    ...
    def get_sales_opportunities
      ...
      parts.each do |part|
        ...
        Auction.where(part_num: part.part_num, active: true).each do |auction|
          @sales_opportunities << auction unless auction.company == current_user || !(auction.bids & current_user.bids).empty? || (!auction.condition.include?(part.condition) && !auction.condition == "All Conditions")
        end
      end
      ## OLD CODE
