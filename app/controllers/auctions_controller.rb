class AuctionsController < ApplicationController
	...
	def index
		...
		@pending_purchases_count = Transaction.where(buyer_id: current_company.id, status: "completed").count
		@purchases_count = Transaction.where(buyer_id: current_company.id).where.not(status: "completed").count
	end
