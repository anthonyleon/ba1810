    ...
    def get_sales_opportunities
      ...
      parts.each do |part|
        ...
        Auction.where(part_num: part.part_num, active: true).each do |auction|
          @sales_opportunities << auction if auction.company != current_user || (auction.bids & current_user.bids).empty? || auction.condition.include?(part.condition) || auction.condition == "All Conditions"
        end
      end
      ## OLD CODE
