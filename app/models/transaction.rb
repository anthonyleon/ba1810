class Transaction < ActiveRecord::Base
  ...
  def parse_webhook(data, bid)
    if data["api_key"]["api_key"] == ENV['ARMOR_PKEY']
      ...
      when 16 # order cancelled
        Notification.notify(bid, bid.seller, "The order ##{self.order_id} for part ##{bid.auction.part_num} has been cancelled.", tx: self)
      ...
      when 15 # shipment details added to order (testing purposes, not really but need to check later) this doesn't mean it was received does it?
        Notification.notify(bid, bid.buyer, "Shipment information for order ##{self.order_id} for #{self.auction.part_num} has been received.", tx: self)
      when 3 #goods shipped to buyer
        Notification.notify(bid, bid.buyer, "Your purchase for part ##{bid.auction.part_num} (order ##{self.order_id}) has been shipped.", tx: self)
      ...
      when 4 # goods received by buyer
        ...
        Notification.notify(bid, self.seller, "Buyer for order ##{self.order_id}, has received shipment. Funds will be released upon approval of part.", tx: self)
        Notification.notify(bid, self.buyer, "Order ##{self.order_id}, has been marked as received. You have 3 days to approve part.", tx: self)
      when 6 # order accepted (ie. funds released from buyer to seller)
        ...
        Notification.notify(bid, bid.seller, "The funds for order ##{self.order_id} have been released from escrow in accordance with your payout preference.", tx: self)
      when 10 # dispute settlement offer has been submitted by either buyer or seller
        self.settlement_offer_submitted
        Notification.notify(bid, self.buyer, "A settlement offer has been submitted to you. Please review.", tx: self)
  ...
  #DISPUTES
      when 3000 # Dispute created
        ...
        Notification.notify(bid, bid.seller, "Buyer for #{bid.auction.part_num}, order ##{self.order_id}, has disputed the transaction.", tx: self)
      ...
      when 3004 # Offer to settle dispute on order accepted
        ...
        Notification.notify(bid, company, "Your settlement offer for order ##{self.order_id} has been accepeted", tx: self)
